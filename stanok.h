...
// блоки что включаются 
 	
	struct MCP23017_PIN *pin_on[MAX_tech_pin];  
	uint8_t  N_pin_on; // 0 - нет блоков на переключение			     

// блоки что выключатся на данном переходе

	struct MCP23017_PIN *pin_off[MAX_tech_pin]; 
	uint8_t  N_pin_off;	

// ожидания события от пина  
	struct MCP23017_PIN *pin_waiting_event[MAX_tech_pin]; 
	uint8_t  N_pin_waiting;

//Время задержки перед переходом к другому переходу.
    int transition_delay; 

//Следующий переход 
struct Tech_transition *next;

	void (*activation_transition)(struct Tech_transition *transition);
};
...
struct Tech_transition *Init_process;
