 ...
 * struct Tech_transition{
 	...
 	*pin_on[MAX_tech_pin];  
	*pin_off[MAX_tech_pin];
	* 
	*pin_waiting_event[MAX_tech_pin]; 
...
/*
 * Иницилизировать тех операцию
 * для запуска циклов переходов 
 */
...
/*
 * Первый переход, включить пневмоциллиндр
 * и дождатся его подвода к заготовке.
 */
struct Tech_transition 
...
// дождатся когда пнемоциллиндр дойдет до цели
.pin_waiting_event = {LIMIT_SWITCH},
.pin_waiting_value = {HIGH},
.N_pin_waiting = 1,
...
.next = &Plate_160x200_n2,
	
	.pin_waiting = pin_waiting_transition,
	.activation_transition = inital_transition,
};
/*
 * Второй переход, включить клапан ваккума -
 * захват новой заготовки и выдержать 
 * предыдущию заготовку в штампе.
 */
struct Tech_transition 
Plate_160x200_n2 = {
	...
	.next = &Plate_160x200_n3,
	
	.pin_waiting = pin_waiting_transition,
	.activation_transition = inital_transition,
};
/*
 * Третий переход, убрать заслонку для 
 * выпадения заготовки, 
