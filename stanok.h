...
struct MCP23017_PIN mcp_gpio[] ={
	...
	{&chip1,7,pin_IN,pin_no_pullUP,pin_HIGH,pin_no_invert}, //pin7 - оптопара вход  транзистор											
 	{&chip1,14,pin_IN,pin_no_pullUP,pin_HIGH,pin_no_invert},//pin8 - оптопара на замыкание
	{&chip1,15,pin_IN,pin_no_pullUP,pin_HIGH,pin_invert}, 	//pin9 - оптопара вход транзистор
...
struct Thermosensors MAX6675={
	...
	.timer_cycle = 1,  //при первом пуске проверить температуру 
...
void check_event(struct ProductCounter *counter){
 ...
 uint8_t bit = counter->pin_counter->bit;
 uint8_t registr = GPIOA|((bit&0b00001000)>>3);		
 uint8_t status_bit = 0b00000001 << (bit&0x7);	

 uint8_t verify_bit = ((counter->pin_counter->chip->registr[registr]) & status_bit)>>(bit&0x7);
	
  if (verify_bit == counter->sequence_of_events[counter->N_positions_in_the_event])
	return; // данные значение не изменилось выходим
