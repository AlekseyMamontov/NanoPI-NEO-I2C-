...
struct tft_screen_panel {
	...
	int16_t cursor_y;
	int16_t color_font;
	int16_t color_background;
	__int8_t font_x;
	__int8_t font_y;
const __int8_t *font_size;
...
void write_tft_command (struct tft_screen_panel *panel,__int8_t value){
...
void write_tft_data (struct tft_screen_panel *panel ,__int8_t value){
...
int write_tft_clear (struct tft_screen_panel *panel ,__int16_t color){

	 MCP23017 *chip = &(panel->tft_model-> chip);
	 
	 int chip_write;	
	__int8_t  color_msb = color >> 8;
	__int8_t  color_lsb = color & 0xFF;
...
int write_tft_clear_fast (struct tft_screen_panel *panel,__int8_t color){
...
int TFT_setWindow (struct tft_screen_panel *panel, __int16_t x0,
									__int16_t y0,
									__int16_t x1,
									__int16_t y1){
 ...
 __int8_t  x0_msb = x0 >> 8;
 __int8_t  x0_lsb = x0 & 0xFF;
 __int8_t  x1_msb = x1 >> 8;
 __int8_t  x1_lsb = x1 & 0xFF;
 __int8_t  y0_msb = y0 >> 8;
 __int8_t  y0_lsb = y0 & 0xFF;
 __int8_t  y1_msb = y1 >> 8;
 __int8_t  y1_lsb = y1 & 0xFF;
 
  	
    __int8_t buf[45] = {OLATA,				// buf[0]  - номер  регистра
...
////////////////////////////////// Write char to Screen Panel ////////////////////////////////////////////

int write_char_tft (struct tft_screen_panel *panel,int syfra){
...
chip->rdwr.nmsgs = 1;		
  ...
  // TFT_setWindow (chip_tft,x,y,x1,y1);
   
   	
 __int8_t  color_msb = panel->color_font >> 8;
 __int8_t  color_lsb = panel->color_font & 0xFF;
 __int8_t  background_msb = panel->color_background >> 8;
 __int8_t  background_lsb = panel->color_background & 0xFF;	
 __int8_t  sizeXfont = panel->font_x;
 __int8_t  sizeYfont = panel->font_y;
 __int8_t  colXbayt = (panel->font_x)/8;


	int  znak = syfra*sizeYfont;
	int len_byte = 1;
	chip->buf[0] = OLATA;

	
		for( __int8_t n=0; n < sizeYfont; n++){
					
			for (__int8_t l=0; l < colXbayt; l++){

				for (__int8_t i=0; i<8 ; i++){
				
					if ((panel->font_size[znak]&(1<<(7-i)))!=0){
