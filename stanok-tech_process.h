...
struct Tech_transition{
 ...
 void 					*data; // любые данные передаваемые в переход через указатель
...
struct Tech_operation{
 ...
 // разные данные для операции через указатель
 void *data;
...
/* Запустить паузу(nanosleep) и данные времени (struct timespec) взять из void *data  
   и включить или выключить устройства */
#define DATA_PAUSE_AND_DEVICE_TRANSITION 11
void Data_Pause_Device_on_off(struct Tech_process *tech_process){
	
  uint8_t status = tech_process->move_to_next_transition;	
  struct timespec *t = (struct timespec *) tech_process->current_operation->data;
  
  if ( t != NULL){
  tech_process->current_transition->transition_delay.tv_sec = t->tv_sec;
  tech_process->current_transition->transition_delay.tv_nsec = t->tv_nsec;
   };
...
#define PAUSE_TIMER_AND_DEVICE_TRANSITION 12
...
#define INIT_SENSOR_DEVICE_TEMPWORK_PAUSE 13
...
#define WAIT_SENSORS_ANTI_RATTLING 14
...
printf("enabled Tech_process ... \n");
 ...
 void (*launch_transition[])(struct Tech_process *)={
	...
	Device_on_off_Check_sensors, // 6		
	Check_sensors_Device_on_off, // 7
	Device_on_off_Pause,		 // 8
	Device_on_off_Pause_timer,	 // 9
	Pause_Device_on_off,		 // 10
    Data_Pause_Device_on_off,	 // 11
	Pause_timer_Device_on_off,   // 12
	Init_Sensor_Device_TempWork_Pause, // 13
	Wait_sensors_anti_rattling, // 14
 };
