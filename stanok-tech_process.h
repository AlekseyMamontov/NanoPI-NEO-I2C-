...
#define TECH_PROCESS_SENSORS_WAIT	0b00000010
#define TECH_PROCESS_DEVICE_ON_OFF	0b00000100
...
struct Tech_pins{
	...
	uint8_t dependcy[MAX_tech_pin]; // зависимость
	uint8_t 				 N_pin; // 	
};
...
tech_process->move_to_next_transition = status_transition;
...
void Pause_timer_transition(struct Tech_process *tech_process){
  struct Tech_transition *transition = tech_process->current_transition;
  if(transition->transition_delay.tv_sec || transition->transition_delay.tv_nsec){	
	  if(tech_process->hide_init_on_tranistion){

			tech_pause_timer(tech_process);

			tech_process->hide_init_on_tranistion = 0;};				
	  }else tech_process->move_to_next_transition &= ~TECH_PROCESS_PAUSE;	  
};
...
/****************************************************************************************
 * Простые переходы	с зависимостью от каких то значений и событий						*											*		*
 ****************************************************************************************/
...
void pthread_Tech_process(void *per_tech){
 ...
 void (*launch_transition[])(struct Tech_process *)={
	 NOP_transition,
	 Device_on_off_transition,
	 Pause_transition,
	 Pause_timer_transition,
	 Check_sensors_transition
	 };
