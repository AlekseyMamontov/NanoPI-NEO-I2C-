...
void tech_pause_clock_nanosleep(struct Tech_process *tech_process){
	...
	if (request.tv_nsec > 999999999) request.tv_nsec = request.tv_nsec-999999999;
	
	if(erorr = clock_nanosleep(CLOCK_REALTIME,TIMER_ABSTIME,&request,NULL)){ 
	printf ("erorr  %d \n", erorr);
...
Tech_set_command(process,
				 SELECTION_OPERATION,
				 (void*)(uint)menu->selected_line->number
				 );	
};
