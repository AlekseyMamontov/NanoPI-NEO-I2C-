...
struct TFT_menu{
	...
	uint8_t *visible_selection_on[MAX_line_menu];
...
// Подготовить информацию для очередного изменения на экране
	if((n+1) < menu->info_block.N_change_block){
	 
// off menu_line - убрать выделение текста
...
// on menu_line - перейти на новую строку и  выделить текст
    n++;
...
//ничего не активируется null функция. 

void tft_no_activation(struct TFT_menu *menu){};
