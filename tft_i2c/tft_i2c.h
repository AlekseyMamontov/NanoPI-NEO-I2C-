...
void tft_data_to_char(struct TFT_info_block *info_block, // структура для отоброжения инфо на tft
 ...
 uint32_t del = tft_data_to_char_table[info_block->N_change_char_text[n_block]]; 

    if(info_block->N_change_char_text[n_block] < length && length == 0 ) 
...
void pthread_tft_panel(void *n_panel){
 ...
 while(panel->command){
	...
	switch (command){
			...
			pause.tv_sec = 0;
			pause.tv_nsec = 120000000;
						
			nanosleep(&pause,NULL); //120ms если не сделать будут артефакты.
