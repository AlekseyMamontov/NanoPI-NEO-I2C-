...
struct TFT_info_block{
	...
	uint8_t *sensors_txt[MAX_change_block];	  // показание температуры, номер пинов
   ...
   void (*Change_InfoBlock[MAX_change_block])(struct TFT_screen_panel *panel, int, int ); // тестовая функция
...
///////////////////////////////////////////////////////////////////////////////////////////////////
//  Обновить число на экране, если цифра и ее положение на экране совпадают то цифру не выводить//
...
void tft_fast_change_number (struct TFT_screen_panel *panel, int n_block,int n_fast_block){
 ...
 tft_printf(panel, panel->info_block[n_block]->change_block[n_fast_block]); // выставляем параметры для  вывода
...
// Походу обновляем данные от датчиков для экрана.	
	...
	for(int r=0; r< panel->info_block[n_block]->N_change_char_text[n_fast_block]; r++){
			...
			per  = panel->info_block[n_block]->sensors_txt[n_fast_block][r];
