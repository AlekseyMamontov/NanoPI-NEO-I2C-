...
struct TFT_info_block{
	...
	uint8_t lock_change[MAX_change_block];
...
uint8_t per,per1;
	...
	panel->info_block[n_block]->lock_change[n_fast_block] = 1;
...
// Перебираем строку, если цифра на экране и полученные цифра от датчика совпадает, символ не выводим	
	...
	panel->info_block[n_block]->lock_change[n_fast_block] = 0;
};	
...
// lock for menu
    ...
    if(menu->info_block.lock_change[n]) return;
