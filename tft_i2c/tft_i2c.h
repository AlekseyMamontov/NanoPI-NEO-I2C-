...
//--------------------------------------------------------------------//
// Блок отвечает за отображение статической и динамическую информацию //
//-------------------------------------------------------------------//
#define MAX_change_block 5  // максимальное количество изменяемых 
							// значений в блоке
struct TFT_info_block{
	...
	uint8_t *sensors_txt[MAX_change_block];
   ...
   uint8_t (*Change_InfoBlock)(struct TFT_screen_panel *panel); // тестовая функция
...
// данные элементы должны быть в начале списка
	   if(panel->info_block[i]->N_fast_change_block == 0) continue;
	   
		for(int s=0; s < panel->info_block[i]->N_fast_change_block; s++){	// N_fast - указывает сколько их в блоке

			  tft_printf(panel, panel->info_block[i]->change_block[s]); // выставляем параметры для  вывода

// Перебираем строку, если на экране и полученные символы от датчика совпадает, символ не выводим
// Походу обновляем данные от датчиков для экрана.
 		  
		  for(int r=0; r< panel->info_block[i]->N_change_char_text[s]; r++){
		
			per  = panel->info_block[i]->sensors_txt[s][r];
			per1 = panel->info_block[i]->change_text[s][r];
			panel->info_block[i]->change_text[s][r] = per;
