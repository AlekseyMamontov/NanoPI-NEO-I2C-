...
struct TFT_info_block{
	...
	struct MCP23017_PIN *pin[MAX_change_block]; // пин для  индикатора, счетчика и т.п. за котором он следит   				    
...
void tft_fast_change_led (struct TFT_screen_panel *panel, int n_block,int n_fast_block){
	...
	uint8_t bit = panel->info_block[n_block]->pin[n_fast_block]->bit;
	uint8_t status_bit = 0b00000001 << (bit&0x7);// создаем маску бита
	
  
  
	uint8_t registr = GPIOA;
	  
/* на удаление - порт GPIOA - двухнаправленный порт - надо подумать

//Исходя из того как выставлен порт на чтение или запись опрашиваем разные регистры.
//IODIRA (1 - in ->GPIOA , 0 -out ->OLATA)
 
	panel->info_block[n_block]->pin[n_fast_block]->modeInOut ? registr = GPIOA : registr = OLATA;
*/
...
// при обмене данных с микросхемой, запись или чтении - эти регистры обновляются
	...
	uint8_t per = panel->info_block[n_block]->pin[n_fast_block]->chip->registr[registr];
...
// Сравнить значение бита с битом отмеченным как включающий и вывести фон под текстом исходя из этого 
	...
	if(per == panel->info_block[n_block]->pin[n_fast_block]->on) {
...
uint8_t n = menu->n_selected_menu_line;
	...
	*menu->info_block.visible_block[n] = 1;	// отобразить на экране изменения 
};
...
uint8_t n = menu->n_selected_menu_line;
	...
	// eсли выделение внизу окна то пройти дальше по списmenu_line_1_visibleку, 
...
// задержка, чтоб сначала стереть выделение при проходе, а втором выделить, так красивее))
		*menu->info_block.visible_block[n] =2;	
