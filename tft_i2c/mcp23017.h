...
#define GPIOA 0x12 //0b00010010 В этом регистре хранятся значения  на портах выставленных как входы
#define GPIOB 0x13 //0b00010011 c помощью регистров IODIRA , IODIRB
...
uint8_t mcp23017_write_array (MCP23017 *chip , uint8_t *buf, uint32_t value){

	struct i2c_msg msg [1] = {{chip->address, 0, value, buf }}; 
	struct i2c_rdwr_ioctl_data rdwr = {msg, 1 };
...
uint8_t mcp23017_registr_set_bit (MCP23017 *chip, uint8_t registr, uint8_t bit,uint8_t value){
	...
	registr = registr|((bit&0b00001000)>>3);
	uint8_t status_bit = 0b00000001 << (bit&0x7);
			
	mcp23017_read_byte(chip,registr);
	
	if(value){value = chip->registr[registr]|status_bit;
		}else{value = chip->registr[registr]&(~status_bit);}
			
	mcp23017_write_byte(chip,registr,value);	
};
...
uint8_t mcp23017_set_bit (MCP23017 *chip, uint8_t bit,uint8_t value){

	return mcp23017_registr_set_bit (chip,OLATA,bit,value);
};
...
int mcp23017_enable_device (struct MCP23017_PIN *pin){
	printf("%x",pin->on);
	return mcp23017_set_bit (pin->chip,pin->bit,pin->on);
};
...
int mcp23017_init_device (struct MCP23017_PIN *pin){
	...
	if(pin->modeInOut&1){mcp23017_set_pullUP(pin->chip,pin->bit,pin->default_value);
						 return mcp23017_set_IPOL(pin->chip,pin->bit,pin->ipol);};
						 	
	return mcp23017_set_bit (pin->chip,pin->bit,pin->default_value);
};
...
uint8_t mcp23017_set_bit_v2 (MCP23017 *chip,uint8_t registr, uint8_t bit,uint8_t value){
	...
	if (mcp23017_open_i2c (chip) < 0){chip->lock_chip = 0;
									  return chip->bus_open;};
