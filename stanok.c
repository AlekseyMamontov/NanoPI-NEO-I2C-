...
#ifndef Stanok
#error -DStanok = ? (1 - V01 vertical Automat; 2 VG01 vertgorizont_Automat 3-G01 gorizontal Automat 4-Semiautomat)
#endif
...
#elif Stanok == 4
#include "Semiautomat_programm_tech_process.h"
#else
#error -DStanok= ?  (1 - V01 vertical Automat; 2 VG01 vertgorizont_Automat 3-G01 gorizontal Automat 4-Semiautomat)
#endif
...
//if(cycle2) Test_operation.launch_transition(&Test_operation);
	...
	if(chip_keys.buffer[0] == 15 && old_status !=15) {		
		...
		old_status = 15;};	
//	if(chip_keys.buffer[0] == 14 && old_status !=14) {tft_set_command(panel,TFT_FAST_CLEAR,NULL); old_status = 14;};
