...
tft_set_command (panel,TFT_INIT,NULL);
tft_set_command (panel,TFT_FAST_CLEAR,NULL);
tft_set_command (panel,TFT_BUILD_INFO_BLOCK,NULL);
tft_set_command (panel,TFT_UPDATE_CHANGE_BLOCK,NULL);

pthread_start_nGPIO (&Stanok_gpio);
gpio_set_command(&Stanok_gpio,INIT_GPIO,NULL);
