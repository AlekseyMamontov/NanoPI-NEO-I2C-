...
/* Эти регистры являются дублирующими друг друга.
...
#define GPPUB 0x0d
/*
 * Помогает определить на какой ножке произошло прерывание,  по умолчанию во всем 
 * регистре 0, произошло прерывание на порту в соответствующем разряде  появится 1
 */
#define INTFA 0x0e
#define INTFB 0x0f
...
#define GPIOB 0x13
/*
 В этом регистре выставляются значения на портах как выходы
  */
#define OLATA 0x14
#define OLATB 0x15
...
MCP23017_write_byte(0x20, OLATA,0xff);
sleep(1);
MCP23017_write_byte(0x20, OLATA,0x00);
sleep(1);
MCP23017_write_byte(0x20, OLATA,0xf0);
sleep(1);
MCP23017_write_byte(0x20, OLATA,0x0f);
sleep(1);
MCP23017_write_byte(0x20, OLATA,0x00);
};
