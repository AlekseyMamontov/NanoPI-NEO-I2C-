...
mcp23017_registr_set_bit (Stanok_CHIP,IODIRA,Stanok_PIN.bit,Stanok_PIN.direction);};

void mcp_init(uint8_t pin){
	
mcp23017_registr_set_bit (Stanok_CHIP,IODIRA,Stanok_PIN.bit,Stanok_PIN.direction); //0-out 1-in

	if(Stanok_PIN.direction&1){
	mcp23017_registr_set_bit (Stanok_CHIP,GPPUA,Stanok_PIN.bit,Stanok_PIN.default_value);
	mcp23017_registr_set_bit (Stanok_CHIP,IPOLA,Stanok_PIN.bit,Stanok_PIN.ipol);
	Stanok_PIN.value = 
	mcp23017_registr_read_bit(Stanok_CHIP,GPIOA,Stanok_PIN.bit); 
	return;};
...
case GET_PIN:				 
	...
	if(n_pin_sensors < MAX_PINs){
		...
		status_read |= gpio->chip[gpio->pin[data->pin.n_pin].n_chip].id_chip;								
...
case GET_PIN_GROUP:		  
	...
	if((n_pin_sensors + data->num_pin) < MAX_PINs){	 	   	
		...
		status_read |= gpio->chip[gpio->pin[data->pin.group[i]].n_chip].id_chip;
...
_set_pin:	 
	 ...
	 status_write |= gpio->chip[gpio->pin[data->pin.n_pin].n_chip].id_chip;
...
_set_pin_group:	 	  	   		
	 if((n_pin_sensors + data->num_pin) < MAX_PINs){
		for (int i=0; i < data->num_pin ; i++){
			...
			status_write |= gpio->chip[gpio->pin[data->pin.group[i]].n_chip].id_chip;
			status_read  |= gpio->chip[gpio->pin[data->pin.group[i]].n_chip].id_chip;
			pin_sensors[n_pin_sensors++] = data->pin.group[i];		
		};
...
case GET_RAW_MAX6675:
	 ...
	 //маска для чипов с каких читать пины которые miso	 
		mask_miso_read |= gpio->chip[gpio->pin[data_cycle.pin.clk_cs_miso[i]].n_chip].id_chip;
