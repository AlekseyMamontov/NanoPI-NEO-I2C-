...
for(uint8_t t=0; t < thermosensor->N_thermal_sensors; t++){				
	...
	if (thermosensor->thermo_raw[t]&0b1000000000000100){
		
	// 4 - нет термопары, 5 - устройства нет 
		
		thermosensor->check_thermocouple[t] = (thermosensor->thermo_raw[t]&0x4)|((thermosensor->thermo_raw[t]&0x8000)>>15);
