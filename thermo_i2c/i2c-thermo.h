...
void pthread_start_thermoblock(struct Thermostat_Blocks *thermoblock){
 ...
 for (int i=0; i< thermoblock->num_thermostat;i++){
	...
	temp_on = stat[i].temperatura_on;
	temp_off = stat[i].temperatura_off;
	
	tft_data_to_char (&stat[i].info_block, 1,
					  stat[i].info_block.N_change_char_text[1],
					  stat[i].temperatura_on);
	tft_data_to_char (&stat[i].info_block, 2,
					  stat[i].info_block.N_change_char_text[2],
					  stat[i].temperatura_off);
 };
