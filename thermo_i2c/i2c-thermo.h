...
*/	
	...
	// Переменная для цикла опроса 0 - выход, 1 - работа в цикле 
	int time_cycle;
...
void pthread_start_thermosensors(struct Thermosensors *thermosensor){
 ...
 //thermosensor->gpio->pin[thermosensor->pin_clk].busy_pin = BUSY_THERMOSENSORS;
 //thermosensor->gpio->pin[thermosensor->pin_cs].busy_pin  = BUSY_THERMOSENSORS; 
 
 for(uint8_t i= 0; i < thermosensor->N_thermal_sensors; i++){
	 
	// thermosensor->gpio->pin[thermosensor->pin_miso[i]].busy_pin = BUSY_THERMOSENSORS;
 };
