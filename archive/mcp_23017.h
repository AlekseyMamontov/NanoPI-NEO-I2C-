...
_Atomic uint8_t lockregistr[0x16];
...
uint8_t mcp23017_write_byte (MCP23017 *chip , uint8_t registr, uint8_t value){
	...
	if ( ioctl(chip->bus_open, I2C_RDWR, &rdwr ) < 0 ){perror("I2C erorr rdwr");return -1;}
...
buf[0]= registr;
                                                 	//выставить регистр который читаем
...
return chip->registr[registr] = buf[0];
};
//-------------------------------------------------------------------------------------//
...
//------------------------------------------------------------------------------------//
//			      		      	Init_block_pin 										  //
// n- количество элементов, i - с какого номера начать; 							  //
//------------------------------------------------------------------------------------//
void mcp23017_init_block_device(struct MCP23017_PIN *pin, uint8_t i, uint8_t n){
	n = n + i;
	for (i; i < n; i++){mcp23017_init_device(&pin[i]);};
};
//------------------------------------------------------------------------------------//
...
int mcp23017_get_registrs_chip (MCP23017 *chip){ 
	...
	struct i2c_rdwr_ioctl_data rdwr = {msg, 2 };                     //поместить массив  блоков (не больше 32 байт в блоке)
		...
		pthread_mutex_lock (&chip->lock_registr_GPIO);														//структуру (на все дается 8192 байт)
