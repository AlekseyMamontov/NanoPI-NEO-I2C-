...
void write_tft_clear_fast (struct TFT_screen_panel *panel){

	uint8_t  color = window->color_background & 0xFF;
...
//! исправить чтоб массив мог любой длины.

int init_TFT_panel(struct TFT_screen_panel *panel){

MCP23017 *chip = &(panel->tft_model->chip); 

panel->buf[0] = GPIOA; //261
...
void tft_menu_down(struct TFT_menu *menu){
	...
	uint8_t n = menu->n_selected_menu_line;
    // uint8_t lock = menu->info_block.N_fast_change_block;
    
	// lock for menu
	if(*menu->info_block.visible_block[n]) return;
	
	//lock for fast_change_block_to_display
	//while (menu->info_block.lock_change) {};
	//menu->info_block.N_fast_change_block = 0;
	...
	// eсли выделение внизу окна то пройти дальше по списку menu_line_visible, 
    ...
    // unlock for fast_change_block_to_display	
	//menu->info_block.N_fast_change_block = lock;
};
...
uint32_t del = 1000000000; 


	if(info_block->N_change_char_text[n_block] < length && length == 0 ) return perror("not correct data to char ");
	
	uint8_t correct = (info_block->N_change_char_text[n_block]) - length;
...
//////////////////////////////////////////////////////////////////////////
/* --------------------------------------------------------------------*
 * TFT панель - подпрограммы для обсуживания TFT панели		 		   *
 *---------------------------------------------------------------------*/
 void func_tft_init (struct TFT_screen_panel *panel){
	...
	mcp23017_write_byte (chip, IOCON, 0b00100000);  
...
#define TFT_CYCLE 1
#define TFT_FAST_CLEAR 2
#define TFT_CLEAR 3
#define TFT_BUILD_INFO_BLOCK 4
#define TFT_UPDATE_CHANGE_BLOCK 5
#define TFT_DISABLE 6
#define TFT_ENABLED 7
#define TFT_INIT 8
#define TFT_PAUSE 255

#define MAX_func_command 10
void *tft_function[MAX_func_command](struct TFT_screen_panel *);

tft_function = {
	NULL,
	fast_change_block_to_display (panel),
	write_tft_clear_fast(panel),
	tft_clear_window,
	build_info_block_to_display,
...
void pthread_tft_panel(void *n_panel){
 ...
 while(panel->command){
	...
	panel->status_command = command;
	
	if (command < MAX_func_command){
		
		tft_function[command-1](panel);
		
		if(command >TFT_CYCLE && command < TFT_INIT+1) panel->command =255:
			
	   panel->status_command = 0;
